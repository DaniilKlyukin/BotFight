...
<b>Целью</b> хакатона является продвижении программирования среди студентов ВУЗов и развитие навыков программирования поисковых алгоритмов.
## О месте проведения
Проведение хакатона планируется в рамках практики по предмету "Информационные технологии и программирование", на кафедре "Прикладная математика и информационные технологии" (г. Ижевск, ИжГТУ им. М.Т. Калашникова).
## О хакатоне (Об игре)
...
7. Чтобы подключться к серверу необходимо, чтобы на сервере была создана карта (Вкладка "Игра" -> "Создать уровень"), после чего можно запускать клиент;
8. Действие игрока определяется в методе DoWork. Каждые 2 секунды сервер отправляет данные карты игрокам, в клиенте автоматически вызывается метод DoWork, в переменной map хранится информация по объектам на карте. На основании этой информации необходимо с помощью return вернуть действие.
 ```
